---
name: aimin-skill
description: 用于按 Aimin 规范初始化 AGENTS.md、CLAUDE.md、.agent/index、.agent/scripts 与按项目类型命中的 admin/tauri/uni 目录、加载规则并新增接口
---
...
- 需要初始化项目内的 `AGENTS.md`、`CLAUDE.md`、`.agent/index/**`、`.agent/scripts/lint.md` 与按项目类型命中的 `.agent/admin/**`、`.agent/tauri/**`、`.agent/uni/**`
- 需要按 Aimin 规范新增接口、类型、常量或项目类型规则

## Workflow

1. 先读 `README.md` 了解当前 skill 包结构
2. 如果任务是初始化项目，优先读 `template/AGENTS.md`、`template/CLAUDE.md`、`template/index/constants.json`、`template/index/utils.json`、`template/scripts/lint.md`，再按项目类型按需读 `template/admin/rules.md`、`template/admin/table.md`、`template/admin/modal.md`、`template/tauri/rules.md`、`template/uni/rules.md`
...
6. 项目侧默认维护 `AGENTS.md`、`CLAUDE.md`、`.agent/index/**`、`.agent/scripts/lint.md`，以及按项目类型命中的 `.agent/admin/**`、`.agent/tauri/**`、`.agent/uni/**` 中一组
7. `/am:init` 只创建根目录 `AGENTS.md`、`CLAUDE.md` 与最小 `.agent/**` 规则集，不创建或修改 `.gitignore`
8. 禁止因为技术栈识别而额外生成 `.agent/comment.md`、`.agent/naming.md`、`.agent/constant.md`、`.agent/api.md`、`.agent/vue.md`、`.agent/unocss.md`；`admin`、`tauri`、`uni` 最多命中一组
